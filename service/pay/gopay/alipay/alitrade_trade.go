...
type TradePreCreate struct {
	...
	TimeOutExpress       string             `json:"timeout_express"`         //可选 交易超时时间
	QrCodeTimeOutExpress string             `json:"qr_code_timeout_express"` //可选 二维码
}
...
type TradeCancel struct {
	...
	OutTradeNo string `json:"out_trade_no"` // 原支付请求的商户订单号,和支付宝交易号不能同时为空,二选一
...
func (this *TradeCloseRsp) IsSuccess() bool {
