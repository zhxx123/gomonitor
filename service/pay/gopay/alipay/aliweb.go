...
import (
	...
	"github.com/zhxx123/gomonitor/service/pay/gopay/encoding"
)
...
type AliWebClient struct {
	appId              string          // app ID
	apiDomain          string          // api 网关
	sellerID           string          // 卖家支付宝ID,非必须
	notifyVerifyDomain string          //回调地址
...
func New(appId, sellerID, aliPublicKey, privateKey string, isProduction bool) (aliWebClient *AliWebClient, err error) {
	...
	if isProduction {
		aliWebClient.apiDomain = kProductionURL
		aliWebClient.notifyVerifyDomain = kProductionMAPIURL
	} else {
		aliWebClient.apiDomain = kSandboxURL
		aliWebClient.notifyVerifyDomain = kSandboxnotifyVerifyDomain
	}
	aliWebClient.SignType = K_SIGN_TYPE_RSA2
	return aliWebClient, nil
}

// 公共头部参数
func (this *AliWebClient) URLValues(param Param) (value url.Values, err error) {
	var p = url.Values{}
...
func (this *AliWebClient) doRequest(method string, param Param, result interface{}) (err error) {
	var buf io.Reader
	if param != nil {
...
func (this *AliWebClient) VerifySign(data url.Values) (ok bool, err error) {
