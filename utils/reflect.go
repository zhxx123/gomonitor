...
func setField(obj interface{}, name string, value interface{}) error {
	...
	if !fieldValue.IsValid() {
		return fmt.Errorf("utils.setField() No such fields: %s in obj ", name)
	}
	...
	if valTypeStr == "float64" && fieldTypeStr == "int" {
		...
		return fmt.Errorf("Provided value type " + valTypeStr + " didn't match obj field type " + fieldTypeStr + " " + name)
	}
	fieldValue.Set(val)
	return nil
}
